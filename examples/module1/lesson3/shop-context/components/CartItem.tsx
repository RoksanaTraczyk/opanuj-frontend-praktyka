...
const CartItemLogo = memo(({id, image}: Pick<CartItemType, 'id' | 'image'>) => (
  <Link to={`/product/${id}`}>
    <img className="max-w-[80px]" src={image} alt="" />
  </Link>
))

const CartItemTitle = memo(({ id, title }: Pick<CartItemType, 'id' | 'title'>) => (
  ...
  return (
    <div className="flex gap-x-4 py-2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500">
      <div className="w-full min-h-[150px] flex items-center gap-x-4">
        <CartItemLogo id={id} image={image}/>
        <div className="w-full flex flex-col">
          ...
          <div className="flex gap-x-2 h-[36px] text-sm">
            <div className="flex flex-1 max-w-[100px] items-center h-full border text-primary font-medium">
              ...
              <div onClick={() => addToCart(item)} className="h-full flex flex-1 justify-center items-center cursor-pointer">
                <IoMdAdd />
              </div>
            </div>
